...
- Multi-feature structure: each feature lives under `features/<feature>/` with its own `agent/` (skills + knowledge + memory, portable), optional `ui_lite/` (browser UI), optional `serve.py` (local dev server), and `ui/` for full Vue apps
- Spec-driven development: all features start with specs before any code
- One task = one commit with passing build + tests
- Skills live in `features/<feature>/skill/` or `features/<feature>/agent/skill/`; each skill follows the Skill Creator workflow (draft → eval → iterate)
...
1. Create the feature folder: `mkdir -p features/<new-feature>/{agent/skill,ui,ui_lite,specs}`
