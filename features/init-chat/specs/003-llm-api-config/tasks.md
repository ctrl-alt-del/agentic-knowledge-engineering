...
- [x] **Task 3.1**: 创建 `ake.example.json` + `ake.schema.json` + `.gitignore` 更新 + `config.ts`
  - 文件: `ake.example.json`, `ake.schema.json`, `.gitignore`, `features/init-chat/ui/src/lib/config.ts`
  - 构建: `npm run build`
  - 测试: `npm test`
...
- [x] **Task 3.2**: 更新 `LlmProvider` 接口（`sendMessage(messages: Message[])` + `sendMessageStream?`）+ `types.ts` 增加 `system` role
  - 文件: `features/init-chat/ui/src/lib/llmProvider.ts`, `features/init-chat/ui/src/lib/types.ts`
  - 构建: `npm run build`
  - 测试: `npm test`

- [x] **Task 3.3**: 更新 `MockResponder` + `ChatContainer` 适配新接口 + 流式支持
  - 文件: `features/init-chat/ui/src/lib/mockResponder.ts` + test, `features/init-chat/ui/src/components/ChatContainer.vue` + test
  - 构建: `npm run build`
  - 测试: `npm test`
...
- [x] **Task 3.4**: 创建 `OpenAiProvider`（fetch-based，SSE 流式解析）
  - 文件: `features/init-chat/ui/src/lib/openAiProvider.ts` + test
  - 构建: `npm run build`
  - 测试: `npm test`

- [x] **Task 3.5**: 创建 `AnthropicProvider`（fetch-based，SSE 流式解析）
  - 文件: `features/init-chat/ui/src/lib/anthropicProvider.ts` + test
  - 构建: `npm run build`
  - 测试: `npm test`
...
- [x] **Task 3.6**: 创建 `skillLoader.ts` + `ConfigStatus.vue` + 更新 `App.vue`
  - 文件: `features/init-chat/ui/src/lib/skillLoader.ts` + test, `features/init-chat/ui/src/components/ConfigStatus.vue` + test, `features/init-chat/ui/src/App.vue`
  - 构建: `npm run dev`
  - 测试: `npm test`
