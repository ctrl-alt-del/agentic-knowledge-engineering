...
### Happy Path
- [x] Given the chat is loaded, when I type a message and press Enter or click Send, then my message appears in the message list right-aligned
- [x] Given I send a message, when the mock responder processes it, then a typing indicator appears briefly, followed by a random response left-aligned
- [x] Given new messages appear, when they overflow the viewport, then the list auto-scrolls to show the latest message
- [x] Given the page loads, when no conversation has started, then a welcome greeting from the assistant is shown

### Edge Cases
- [x] Given the input is empty, when I try to send, then nothing happens (send is blocked)
- [x] Given the input is whitespace-only, when I try to send, then nothing happens
- [x] Given long messages (200+ chars), when rendered, then the bubble expands gracefully without breaking layout
- [x] Given rapid sends, when multiple messages are sent quickly, then each gets its own independent response
