...
- [ ] **Task 1.1**: Scaffold Vue 3 + TypeScript + Tailwind CSS project in `ui/` + create `README.md`
  - Files: `ui/` (entire Vite scaffold), `README.md`
  - Build: `cd ui && npm install && npm run dev`
  - Tests: `cd ui && npx vitest run` (no tests yet, just verify setup)

## Block 2: Core Logic

- [ ] **Task 2.1**: Create `LlmProvider` interface + `MockResponder` implementation
  - Files: `ui/src/lib/llmProvider.ts`, `ui/src/lib/mockResponder.ts`
...
- [ ] **Task 3.1**: Build `MessageBubble` component
  - Files: `ui/src/components/MessageBubble.vue`
  - Build: `cd ui && npm run build`
  - Tests: `cd ui && npm test` (render tests for user/assistant roles, content, edge cases)

- [ ] **Task 3.2**: Build `ChatInput` component
  - Files: `ui/src/components/ChatInput.vue`
  - Build: `cd ui && npm run build`
  - Tests: `cd ui && npm test` (input binding, submit, empty blocking)

- [ ] **Task 3.3**: Build `MessageList` component
  - Files: `ui/src/components/MessageList.vue`
  - Build: `cd ui && npm run build`
  - Tests: `cd ui && npm test` (renders list, auto-scroll)

## Block 4: Integration

- [ ] **Task 4.1**: Build `ChatContainer` + wire into `App.vue` with provider injection
  - Files: `ui/src/components/ChatContainer.vue`, `ui/src/App.vue`
  - Build: `cd ui && npm run build`
