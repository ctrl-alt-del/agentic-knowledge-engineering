...
- [ ] **Task 3.1**: 创建 `ake.example.json` + `ake.schema.json` + `.gitignore` 更新 + `config.ts`
...
- [ ] **Task 3.2**: 更新 `LlmProvider` 接口（`sendMessage(messages: Message[])` + `sendMessageStream?`）+ `types.ts` 增加 `system` role
  - 文件: `ui/src/lib/llmProvider.ts`, `ui/src/lib/types.ts`
  - 构建: `npm run build`
  - 测试: `npm test`

- [ ] **Task 3.3**: 更新 `MockResponder` + `ChatContainer` 适配新接口 + 流式支持
...
- [ ] **Task 3.4**: 创建 `OpenAiProvider`（fetch-based，SSE 流式解析）
  - 文件: `ui/src/lib/openAiProvider.ts` + test
  - 构建: `npm run build`
  - 测试: `npm test`

- [ ] **Task 3.5**: 创建 `AnthropicProvider`（fetch-based，SSE 流式解析）
...
- [ ] **Task 3.6**: 创建 `skillLoader.ts` + `ConfigStatus.vue` + 更新 `App.vue`
...
- [ ] **Task 3.7**: 编写 takeaways.md、更新 MEMORY.md、specs/index.md、README.md、AGENTS.md
